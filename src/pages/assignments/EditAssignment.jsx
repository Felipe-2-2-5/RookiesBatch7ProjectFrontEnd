...
const EditAssignment = () => {
    ...
    const getAssignmentById = async () => {
      try {
        ...
        if (response) {
          ...
          const initialUser = response.assignedTo
            ? response.assignedTo.userName
            : "";
          setInitialValue({
            asset: initialAsset,
            user: initialUser,
  ...
  useEffect(() => {
    ...
    if (touched.assignedDate) {
      if (!selectedAssignment.assignedDate) {
        ...
        const assignedDateOnly = formatDateOnly(
        ...
        if (assignedDateOnly < todayDateOnly) {
          ...
          !(selectedAssignment.assignedDate instanceof Date) ||
          isNaN(selectedAssignment.assignedDate.getTime())
        ) {
  ...
  const isSingleFieldChanged = () => {
    ...
    const assignedDateChanged =
      new Date(initialValue.assignedDate).getTime() !==
      new Date(currentValue.assignedDate).getTime();
    const noteChanged = initialValue.note !== currentValue.note;

    return (
      [assetChanged, userChanged, assignedDateChanged, noteChanged].filter(
        Boolean
      ).length !== 0
    );
  };
